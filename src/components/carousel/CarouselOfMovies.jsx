...
export function CarouselOfMovies ({ movies, mediaType }) {
  const { carouselRef, isFirst, handleSlide, isScrollNeeded, handleScrollNeeded } = useCarousel({ type: 'movie' })
  ...
  return (
    <>
      <div className='w-full relative group '>
        ...
        <div className='w-full overflow-x-scroll scroll-smooth snap-x snap-mandatory gap-[0.7rem] flex justify-between transition-transform duration-500' style={{ scrollbarWidth: 'none' }}
        ref={carouselRef}>
          { movies?.map(movie => {
            return (
              <MediaCard mediaType={mediaType} key={movie.id} media={movie} handleScrollNeeded={handleScrollNeeded}
